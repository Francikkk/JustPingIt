...
class AppPaths:
    ...
    Attributes:
        dev_mode (bool): Indicates whether the application is running in development mode.
        base_path (str): The base path of the application, determined dynamically.
        icon_filename (str): The filename of the application icon.
        db_filename (str): The filename of the database file.
    Methods:
        _get_base_path():
            Determines the base path of the application. If running in a PyInstaller
            bundle, it uses the `_MEIPASS` attribute; otherwise, it uses the directory
            of the current script.
        get_icon_path():
            Constructs and returns the full path to the application icon file.
        get_db_path():
            Constructs and returns the full path to the database file. In development
            mode, it uses the base path. In production mode, it uses the application
    ...
    def __init__(self) -> None:
        ...
        Attributes:
            dev_mode (bool): Indicates whether the application is running in development mode.
                             It is set to True if the '_MEIPASS' attribute is not present in the sys module.
            base_path (str): The base path of the application, determined by the `_get_base_path` method.
            icon_filename (str): The filename of the application icon, default is "JPI.ico".
            db_filename (str): The filename of the database file, default is "ping_log.db".
        """
    ...
    def get_db_path(self) -> str:
        ...
        If the application is in development mode (`dev_mode` is True), the database
        path is constructed using the base path and database filename. Otherwise, the
        database path is located in the application's writable AppData directory.
